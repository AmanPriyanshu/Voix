...
def generate_trends(n=2, epsilon=0.925, processing_limit=100):
	df = pd.read_csv('posts.csv')
	df = df.values
	df = df.T[2:].T
	df = df[-processing_limit:]

	df += np.random.laplace(loc=0.0, scale=1/epsilon, size=df.shape)

	kmeans = KMeans(n_clusters=n, random_state=0).fit(df)
	results = kmeans.cluster_centers_
	indexes = []
	for result in results:
		indexes.append(np.argmin(np.mean(np.abs(result - df), axis=1)))
